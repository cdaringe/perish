...
test('unhandled-2, invalid errrr', t => {
  ...
  r.on('exit', code => {
    ...
    t.ok(
      errMsg.match(/unhandled error detected/),
      'error inserted when no error present'
    )
    t.equals(code, 1, 'exit code 1')
    t.end()
  })
...
test('unhandled-5, adds message when stacktrace does not include it', t => {
  ...
  r.on('exit', code => {
    t.ok(
      errMsg.match(/unhandled-5-message-logged/),
      'messaged logged in stacktrace mode'
    )
